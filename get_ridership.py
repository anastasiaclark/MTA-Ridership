        ...
        if subway_obj!=[]:
            ...
            for i in subways:
                ...
                if train_match:
                    ...
                    if train_match:
                        trains.append(train_match.group().split('.png')[0])## in one of the links 2 train is empty in alt= pattern;
            trainst_str=' '.join(trains)                                   ## use image of the train service instead    
        else:
            trainst_str='None'
        cells=row.findAll('td')
        table_row=[" ".join(c.get_text().strip('\t\n\r').split()) for c in cells]## removes more than one space in between strings
...
updates=updates.drop(['Station (alphabetical by borough)', 'trains','Station (alphabetical by borough)_y', 'trains_y'],1)
